...
function ECPairFactory(ecc) {
    ...
    function makeRandom(options) {
        ...
        if (options === undefined)
            options = {};
        const rng = options.rng || ((v) => Buffer.from((0, utils_1.randomBytes)(v)));
        let d;
        do {
            d = rng(32);
    ...
    class ECPair {
        ...
        sign(hash, _lowR) {
            if (!this.__D)
                throw new Error('Missing private key');
            return Buffer.from(ecc.sign(hash, this.__D, (0, utils_1.randomBytes)(32)));
        }
        signSchnorr(hash) {
            if (!this.privateKey)
                throw new Error('Missing private key');
            ...
            return Buffer.from(ecc.signSchnorr(hash, this.privateKey, (0, utils_1.randomBytes)(32)));
        }
        ...
        tweakFromPublicKey(t) {
            ...
            return fromPublicKey(Buffer.concat([parityByte, tweakedPublicKey.xOnlyPubkey]), { compressed: this.compressed });
        }
